...
object StreamingExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkStreamingContext = new StreamingContext(sparkContext, Milliseconds(500))
    ...
    actorsOfMovies.print(1)
    movieRatingEvents.print(1)
  ...
  def task1(actorsOfMovies: InputDStream[String]): DStream[(String, Long)] = {
    ...
    def updateActorCounter(newCounts: Seq[Long], currentCount: Option[Long]): Option[Long] = {
      val newCount = newCounts.foldLeft(currentCount.getOrElse(0L))(_ + _)
      Some(newCount)
    }
    ...
    actorsOfMovies
      ...
      .reduceByKey(_+ _)
      .updateStateByKey(updateActorCounter)
  }
  ...
  /**
    ...
    * @return a DStream of:
    *         - actor name,
    *         - number of votes since the beginning of time for the movies where the actor plays,
  ...
  def task2(actorsOfMovies: InputDStream[String], movieRatingEvents: InputDStream[String]): DStream[(String, Long, Float)] = {
    ...
    def updateActorVotes(newCounts: Seq[(Long, Long)], currentCount: Option[(Long, Long)]): Option[(Long, Long)] = {
      val newCount = newCounts.foldLeft(currentCount.getOrElse((0L, 0L)))((vote1, vote2) => (vote1._1 + vote2._1, vote1._2 + vote2._2))
      Some(newCount)
    }

    val movieVotes = movieRatingEvents
      .map(line => line.split("\t"))
      .map(parts => (parts(1), (parts(0).toLong, 1L)))
      .reduceByKey((x, y) => (x._1 + y._1, x._2 + y._2)) // key: movie; value1: rate, value2: vote

    val actors = actorsOfMovies
      .map(line => line.split("\t"))
      .map(parts => (parts(1), parts(0))) // movie, actor

    actors
      .join(movieVotes) // join by movie
      .map(row => (row._2._1, row._2._2))
      .reduceByKey((x, y) => (x._1 + y._1, x._2 + y._2)) // key: actor; value1: rate, value2: vote
      .updateStateByKey(updateActorVotes)
      .map(r => (r._1, r._2._2 * 1000, r._2._1.toFloat / r._2._2))
  }
  ...
  def task3(actorRatingsSinceBeginningOfTime: DStream[(String, Long, Float)],
    ...
    val filteredBusyActors = numberOfMoviesByActors
      .filter(d => d._2 >= minimumNumberOfMovies) // actor, numberOfMovies

    actorRatingsSinceBeginningOfTime
      .map(a => (a._1, (a._2, a._3))) // actor, (numberOfVotes, rate)
      .join(filteredBusyActors)
      .map(r => (r._1, r._2._1._2, r._2._2)) // actor, rate, numberOfMovies
  }
